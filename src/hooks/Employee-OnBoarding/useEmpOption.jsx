...
const ExpenseApproverOptions = expenseApproverList?.approvers?.map((item) => {
  return {
    value: item._id,
    label: `${item.first_name} ${item.last_name} (${item.profile.filter(role => 
      ["Super-Admin", "Delegate-Super-Admin", "Department-Head", "Delegate-Department-Head", "HR", "Manager"].includes(role)
    ).join(", ")})`
  };
});
